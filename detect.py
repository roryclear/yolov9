...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights='yolo.pt', device=torch.device('cpu'), dnn=False, data=None, fp16=False, fuse=True):
        ...
        for i in range(len(model.model)):
          if model.model[i].__class__.__name__ == 'Conv':
          ...
          elif model.model[i].__class__.__name__ == 'AConv':
            new_obj = AConv()
            for k, v in vars(model.model[i]).items(): setattr(new_obj, k, v)
            model.model[i] = new_obj
          elif model.model[i].__class__.__name__ == 'ELAN1':
            new_obj = ELAN1()
            for k, v in vars(model.model[i]).items(): setattr(new_obj, k, v)
            model.model[i] = new_obj
          elif model.model[i].__class__.__name__ == 'RepNCSPELAN4':
            new_obj = RepNCSPELAN4()
            for k, v in vars(model.model[i]).items(): setattr(new_obj, k, v)
            model.model[i] = new_obj
          elif model.model[i].__class__.__name__ == 'SPPELAN':
            new_obj = SPPELAN()
            for k, v in vars(model.model[i]).items(): setattr(new_obj, k, v)
            model.model[i] = new_obj
          elif model.model[i].__class__.__name__ == 'Concat':
            new_obj = Concat()
            for k, v in vars(model.model[i]).items(): setattr(new_obj, k, v)
            model.model[i] = new_obj
            ...
            for k, v in vars(model.model[i]).items(): setattr(new_obj, k, v)
            model.model[i] = new_obj
          elif model.model[i].__class__.__name__ == 'CBLinear':
            new_obj = CBLinear()
            for k, v in vars(model.model[i]).items(): setattr(new_obj, k, v)
            model.model[i] = new_obj
          elif model.model[i].__class__.__name__ == 'CBFuse':
            new_obj = CBFuse()
            for k, v in vars(model.model[i]).items(): setattr(new_obj, k, v)
            model.model[i] = new_obj
          elif model.model[i].__class__.__name__ == 'Silence':
            new_obj = Silence()
            for k, v in vars(model.model[i]).items(): setattr(new_obj, k, v)
            model.model[i] = new_obj
          else:
