...
class RepNCSP():
    ...
    def __init__(self, in_ch=1, out_ch=1, in_ch2=1, out_ch2=1, in_ch3=1, out_ch3=1, n=3):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
...
class RepNCSPELAN4():
    ...
    def __init__(self, in_ch=1, out_ch=1, in_ch2=1, out_ch2=1, ch3=1, in_ch4=1, out_ch4=1, in_ch5=1, out_ch5=1,\
        ...
        self.cv2[0] = RepNCSP(in_ch2, out_ch2, ch3, ch3, in_ch4, out_ch4, n)
        ...
        self.cv3[0] = RepNCSP(in_ch2, out_ch2, ch3, ch3, in_ch4, out_ch4, n)
...
class DDetect():
    ...
    def __init__(self, nc=80, ch=(), inplace=True):  # detection layer
        ...
        self.cv2[2][0] = Conv(in_channels=128, out_channels=64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), groups=1, bias=True)
...
if __name__ == "__main__":
  for size in ["t", "s", "m", "c", "e"]:
    ...
    if size == "t":
      ...
      model.model[18] = RepNCSPELAN4(144, 96, 48, 24, 48, 24, 24, 48, 48, 192, 96)
      ...
      model.model[21] = RepNCSPELAN4(192, 128, 64, 32, 64, 32, 32, 64, 64, 256, 128)
      ...
      model.model[4] = RepNCSPELAN4(64*2, 64*2, 32*2, 16*2, 32*2, 16*2, 16*2, 32*2, 32*2, 128*2, 64*2)
      model.model[5] = AConv(in_channels=64*2, out_channels=96*2, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), groups=1, bias=True)
      model.model[6] = RepNCSPELAN4(96*2, 96*2, 48*2, 24*2, 48*2, 24*2, 24*2, 48*2, 48*2, 192*2, 96*2)
      model.model[7] = AConv(in_channels=96*2, out_channels=128*2, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), groups=1, bias=True)
      model.model[8] = RepNCSPELAN4(128*2, 128*2, 64*2, 32*2, 64*2, 32*2, 32*2, 64*2, 64*2, 256*2, 128*2)
      model.model[9] = SPPELAN(ch0=256, ch1=128, ch2=512, ch3=256)
      model.model[10] = Upsample()
      model.model[11] = Concat(f=[-1, 6])
      model.model[12] = RepNCSPELAN4(224*2, 96*2, 48*2, 24*2, 48*2, 24*2, 24*2, 48*2, 48*2, 192*2, 96*2)
      model.model[13] = Upsample()
      model.model[14] = Concat(f=[-1, 4])
      model.model[15] = RepNCSPELAN4(160*2, 64*2, 32*2, 16*2, 32*2, 16*2, 16*2, 32*2, 32*2, 128*2, 64*2)
      model.model[16] = AConv(in_channels=64*2, out_channels=48*2, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), groups=1, bias=True)
      model.model[17] = Concat(f=[-1, 12])
      model.model[18] = RepNCSPELAN4(144*2, 96*2, 48*2, 24*2, 48*2, 24*2, 24*2, 48*2, 48*2, 192*2, 96*2)
      model.model[19] = AConv(in_channels=96*2, out_channels=64*2, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), groups=1, bias=True)
      model.model[20] = Concat(f=[-1, 9])
      model.model[21] = RepNCSPELAN4(192*2, 128*2, 64*2, 32*2, 64*2, 32*2, 32*2, 64*2, 64*2, 256*2, 128*2)
      model.model[22] = DDetect()
      ...
      model.model[22].cv2[1][0] = Conv(in_channels=96*2, out_channels=64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), groups=1, bias=True)
      ...
      model.model[22].cv2[2][0] = Conv(in_channels=128*2, out_channels=64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), groups=1, bias=True)
      ...
      model.model[22].cv3[0][0] = Conv(in_channels=64*2, out_channels=128, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), groups=1, bias=True)
      model.model[22].cv3[0][1] = Conv(in_channels=128, out_channels=128, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), dilation=(1, 1), groups=1, bias=True)
      model.model[22].cv3[0][2] = nn.Conv2d(in_channels=128, out_channels=80, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0), groups=1, dilation=(1, 1), bias=True)

      model.model[22].cv3[1][0] = Conv(in_channels=96*2, out_channels=128, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), groups=1, bias=True)
      model.model[22].cv3[1][1] = Conv(in_channels=128, out_channels=128, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), dilation=(1, 1), groups=1, bias=True)
      model.model[22].cv3[1][2] = nn.Conv2d(in_channels=128, out_channels=80, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0), groups=1, dilation=(1, 1), bias=True)

      model.model[22].cv3[2][0] = Conv(in_channels=128*2, out_channels=128, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), groups=1, bias=True)
      model.model[22].cv3[2][1] = Conv(in_channels=128, out_channels=128, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), dilation=(1, 1), groups=1, bias=True)
      model.model[22].cv3[2][2] = nn.Conv2d(in_channels=128, out_channels=80, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0), groups=1, dilation=(1, 1), bias=True)
      ...
      model.model[2] = RepNCSPELAN4(128, 128, 32*2, 16*2, 32*2, 16*2, 16*2, 32*2, 32*2, 256, 256, n=1)
      model.model[3] = ADown()
      model.model[4] = RepNCSPELAN4(256, 256, 128, 64, 128, 64, 64, 128, 128, 512, 512, n=1)
      model.model[5] = ADown(ch0=256)
      model.model[6] = RepNCSPELAN4(512, 512, 256, 128, 256, 128, 128, 256, 256, 1024, 512, n=1)
      model.model[7] = ADown(ch0=256)
      model.model[8] = RepNCSPELAN4(512, 512, 256, 128, 256, 128, 128, 256, 256, 1024, 512, n=1)
      model.model[9] = SPPELAN(ch0=512, ch1=256, ch2=1024, ch3=512)
      model.model[10] = Upsample()
      model.model[11] = Concat(f=[-1, 6])
      model.model[12] = RepNCSPELAN4(1024, 512, 256, 128, 256, 128, 128, 256, 256, 1024, 512, n=1)
      model.model[13] = Upsample()
      model.model[14] = Concat(f=[-1, 4])
      model.model[15] = RepNCSPELAN4(1024, 256, 128, 64, 128, 64, 64, 128, 128, 512, 256, n=1)
      model.model[16] = ADown(ch0=128)
      model.model[17] = Concat(f=[-1, 12])
      model.model[18] = RepNCSPELAN4(768, 512, 256, 128, 256, 128, 128, 256, 256, 1024, 512, n=1)
      model.model[19] = ADown(ch0=256)
      model.model[20] = Concat(f=[-1, 9])
      model.model[21] = RepNCSPELAN4(1024, 512, 256, 128, 256, 128, 128, 256, 256, 1024, 512, n=1)
      
      model.model[22] = DDetect()
      model.model[22].cv2[0][0] = Conv(in_channels=256, out_channels=64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), groups=1, bias=True)
      model.model[22].cv2[0][1] = Conv(in_channels=64, out_channels=64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), groups=4, bias=True)
      model.model[22].cv2[0][2] = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0), groups=4, bias=True)

      model.model[22].cv2[1][0] = Conv(in_channels=512, out_channels=64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), groups=1, bias=True)
      model.model[22].cv2[1][1] = Conv(in_channels=64, out_channels=64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), groups=4, bias=True)
      model.model[22].cv2[1][2] = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0), groups=4, bias=True)

      model.model[22].cv2[2][0] = Conv(in_channels=512, out_channels=64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), groups=1, bias=True)
      model.model[22].cv2[2][1] = Conv(in_channels=64, out_channels=64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), groups=4, bias=True)
      model.model[22].cv2[2][2] = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0), groups=4, bias=True)


      model.model[22].cv3[0][0] = Conv(in_channels=256, out_channels=256, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), groups=1, bias=True)
      model.model[22].cv3[0][1] = Conv(in_channels=256, out_channels=256, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), dilation=(1, 1), groups=1, bias=True)
      model.model[22].cv3[0][2] = nn.Conv2d(in_channels=256, out_channels=80, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0), groups=1, dilation=(1, 1), bias=True)

      model.model[22].cv3[1][0] = Conv(in_channels=512, out_channels=256, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), groups=1, bias=True)
      model.model[22].cv3[1][1] = Conv(in_channels=256, out_channels=256, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), dilation=(1, 1), groups=1, bias=True)
      model.model[22].cv3[1][2] = nn.Conv2d(in_channels=256, out_channels=80, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0), groups=1, dilation=(1, 1), bias=True)

      model.model[22].cv3[2][0] = Conv(in_channels=512, out_channels=256, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), groups=1, bias=True)
      model.model[22].cv3[2][1] = Conv(in_channels=256, out_channels=256, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), dilation=(1, 1), groups=1, bias=True)
      model.model[22].cv3[2][2] = nn.Conv2d(in_channels=256, out_channels=80, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0), groups=1, dilation=(1, 1), bias=True)
      ...
      model.model[3] = RepNCSPELAN4(128, 128, 32*2, 16*2, 32*2, 16*2, 16*2, 32*2, 32*2, 256, 256, n=2)
      model.model[4] = ADown(ch0=128)
      model.model[5] = RepNCSPELAN4(256, 256, 128, 64, 128, 64, 64, 128, 128, 512, 512, n=2)
      model.model[6] = ADown(ch0=256)
      model.model[7] = RepNCSPELAN4(512, 512, 256, 128, 256, 128, 128, 256, 256, 1024, 1024, n=2)
      model.model[8] = ADown(ch0=512)
      model.model[9] = RepNCSPELAN4(1024, 512, 256, 128, 256, 128, 128, 256, 256, 1024, 1024, n=2)
      ...
      model.model[19] = RepNCSPELAN4(128, 128, 64, 32, 64, 32, 32, 64, 64, 256, 256, n=2)
      model.model[20] = ADown(ch0=128)
      model.model[21] = CBFuse(f=[12, 13, 14, -1], idx=[2, 2, 2])
      model.model[22] = RepNCSPELAN4(256, 256, 128, 64, 128, 64, 64, 128, 128, 512, 512, n=2)
      model.model[23] = ADown(ch0=256)
      model.model[24] = CBFuse(f=[13, 14, -1], idx=[3, 3])
      model.model[25] = RepNCSPELAN4(512, 512, 256, 128, 256, 128, 128, 256, 256, 1024, 1024, n=2)
      model.model[26] = ADown(ch0=512)
      model.model[27] = CBFuse(f=[14, -1], idx=[4])
      model.model[28] = RepNCSPELAN4(1024, 512, 256, 128, 256, 128, 128, 256, 256, 1024, 1024, n=2)
      model.model[29] = SPPELAN(ch0=1024, ch1=256, ch2=1024, ch3=512, f=28)
      model.model[30] = Upsample()
      model.model[31] = Concat(f=[-1, 25])
      model.model[32] = RepNCSPELAN4(1536, 512, 256, 128, 256, 128, 128, 256, 256, 1024, 512, n=2)
      model.model[33] = Upsample()
      model.model[34] = Concat(f=[-1, 22])
      model.model[35] = RepNCSPELAN4(1024, 256, 128, 64, 128, 64, 64, 128, 128, 512, 256, n=2)
      model.model[36] = ADown(ch0=128)
      model.model[37] = Concat(f=[-1, 32]) 
      model.model[38] = RepNCSPELAN4(768, 512, 256, 128, 256, 128, 128, 256, 256, 1024, 512, n=2)
      model.model[39] = ADown(ch0=256)
      model.model[40] = Concat(f=[-1, 29])
      model.model[41] = RepNCSPELAN4(1024, 1024, 512, 256, 512, 256, 256, 512, 512, 2048, 512, n=2)
